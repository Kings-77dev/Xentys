    ...
    <>
      <section className="bg-navy pt-36 pb-16" aria-labelledby="consult-heading">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-[120px]">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-amber/15 rounded-full text-amber text-xs font-semibold tracking-widest uppercase mb-6">
      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-[120px] py-16">
        <div className="grid lg:grid-cols-[1fr_380px] gap-16 items-start">
          ...
          <div>
            {state === "idle" ? (
              <div className="bg-white border border-border rounded-3xl p-12">
                ...
                <form className="flex flex-col gap-6" onSubmit={(e) => { e.preventDefault(); setState("confirmed"); }}>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[{ id: "c-name", label: "Your name", ph: "Thomas van der Berg", auto: "name" }, { id: "c-co", label: "Company", ph: "Acme Industrial BV", auto: "organization" }].map((f) => (
                      <div key={f.id} className="flex flex-col gap-2">
                        ...
                        <input id={f.id} required autoComplete={f.auto} placeholder={f.ph} className="h-12 px-4 rounded-lg border-[1.5px] border-border focus:border-amber focus:outline-none text-sm transition-colors" />
                      </div>
                    ))}
                  </div>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[{ id: "c-title", label: "Your job title", ph: "Procurement Director", auto: "organization-title" }, { id: "c-email", label: "Work email", ph: "thomas@acme.nl", auto: "email", type: "email" }].map((f) => (
                      <div key={f.id} className="flex flex-col gap-2">
                        <label htmlFor={f.id} className="text-sm font-semibold text-text-primary">{f.label} <span className="text-red-500">*</span></label>
                        <input id={f.id} type={f.type ?? "text"} required autoComplete={f.auto} placeholder={f.ph} className="h-12 px-4 rounded-lg border-[1.5px] border-border focus:border-amber focus:outline-none text-sm transition-colors" />
                      </div>
                    ))}
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <input id="c-phone" type="tel" required autoComplete="tel" placeholder="+31 6 …" className="h-12 px-4 rounded-lg border-[1.5px] border-border focus:border-amber focus:outline-none text-sm transition-colors" />
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <input id="c-role" required placeholder="e.g. Senior Buyer, Category Manager, CPO" className="h-12 px-4 rounded-lg border-[1.5px] border-border focus:border-amber focus:outline-none text-sm transition-colors" />
                  </div>
                  <fieldset>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <select id="c-timeline" required className="h-12 px-4 rounded-lg border-[1.5px] border-border focus:border-amber focus:outline-none text-sm transition-colors appearance-none bg-white"
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea id="c-notes" rows={4} className="px-4 py-3 rounded-lg border-[1.5px] border-border focus:border-amber focus:outline-none text-sm transition-colors resize-y" placeholder="Sector, team size, seniority, any context that'll help us prepare for the call…" />
                  </div>
                  <button type="submit" className="w-full h-12 bg-amber text-navy font-semibold rounded-full hover:scale-[1.02] hover:shadow-md transition-all">Submit brief →</button>
              ...
              <div className="flex flex-col gap-4 p-12 bg-off-white rounded-2xl border border-border">
                <div className="w-14 h-14 bg-amber/10 rounded-full flex items-center justify-center text-amber-text">
          ...
          <aside className="flex flex-col gap-6 sticky top-20" aria-label="What happens next">
            ...
            <div className="bg-off-white rounded-2xl p-6 border border-border border-l-4 border-l-amber">
            ...
            <div className="bg-off-white rounded-2xl p-6 border border-border">
